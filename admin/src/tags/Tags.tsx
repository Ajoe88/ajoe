...
import { TagsUpdateInput } from "../api/tags/TagsUpdateInput";
...
export const Tags = (): React.ReactElement => {
  ...
  const [deleteEntity] = useMutation<TTags, AxiosError>(
    async (data) => {
  ...
  ] = useMutation<TTags, AxiosError, TagsUpdateInput>(async (data) => {
    const response = await api.patch(`${"/api/tags"}/${id}`, data);
    return response.data;
  });

  const handleSubmit = React.useCallback(
    (values: TagsUpdateInput) => {
      void update(values);
    },
    [update]
  );
